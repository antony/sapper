...
export async function build({
	...
	const { client, server, serviceworker } = await create_compilers(bundler, false, cwd, src, dest, false);
	...
	if (legacy) {
		...
		const { client } = await create_compilers(bundler, false, cwd, src, dest, false);
